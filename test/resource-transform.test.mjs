...
} from "../src/core/wgsl/buildResourceTransformPlan.js";
...
test("detail transform lowers one physical array binding with fixed semantic layers", () =>
{
    ...
    ])
    {
        ...
        const bindingPlan = CjsFormatWebgpu.buildWgslBindingPlan([ fixture.ir ], {
            resourceTransformPlan
        });
        const shader = CjsFormatWebgpu.buildWgsl(fixture.ir, {
            bindingPlan,
            resourceTransformPlan
        });
...
test("WGSL set validates removed transform inputs within their owning pass", () =>
{
    ...
    const transformedBindingPlan = CjsFormatWebgpu.buildWgslBindingPlan(
        [ transformedFixture.ir ],
        { resourceTransformPlan }
    );
    const transformedShader = CjsFormatWebgpu.buildWgsl(transformedFixture.ir, {
        bindingPlan: transformedBindingPlan,
        resourceTransformPlan
    });

    const ordinaryFixture = detailFixture(3);
    const ordinaryBindingPlan = CjsFormatWebgpu.buildWgslBindingPlan([ ordinaryFixture.ir ]);
    const ordinaryShader = CjsFormatWebgpu.buildWgsl(ordinaryFixture.ir, {
