...
{
    return CjsFormatWebgpu.buildShaderIr(
...
{
    return CjsFormatWebgpu.buildShaderIr(sortInnerFixture(minorVersion), {
...
{
    return CjsFormatWebgpu.buildShaderIr(sortComputeFixture(minorVersion), {
...
test("scalar-word profiles reject inconsistent declaration and binding return-type mirrors", () =>
{
    ...
    for (const [ profileName, makeFixture ] of profiles)
    {
        ...
        {
            const program = structuredClone(CjsFormatWebgpu.buildShaderIr(makeFixture(), {
...
test("canonical exact compute profiles preserve genuine baseline WGSL hashes", () =>
{
    ...
    {
        const code = CjsFormatWebgpu.buildWgsl(program).code;
        assert.equal(
...
test("particle clear effect proof keeps reset effect-only and emits both exact profiles", () =>
{
    for (const minorVersion of [ 0, 1 ])
    {
        ...
        assert.throws(
            () => CjsFormatWebgpu.buildWgsl(reset),
            /whole-effect R32_SINT companion proof/u
        );
        assert.throws(
            () => CjsFormatWebgpu.buildWgslBindingPlan([ reset ]),
            /whole-effect R32_SINT companion proof/u
        );
        ...
        const resetPlan = CjsFormatWebgpu.buildWgslBindingPlan(
            [ reset ],
            { effectProfileProof: resetProof }
        );
        const clearPlan = CjsFormatWebgpu.buildWgslBindingPlan(
            [ clear ],
            { effectProfileProof: clearProof }
        );
        const resetCode = CjsFormatWebgpu.buildWgsl(reset, {
            bindingPlan: resetPlan,
            effectProfileProof: resetProof
        }).code;
        const clearCode = CjsFormatWebgpu.buildWgsl(clear, {
...
test("particle clear proof is opaque, program-bound, and reflection-bound", () =>
{
    ...
    assert.throws(
        () => CjsFormatWebgpu.buildWgsl(reset, { effectProfileProof: {} }),
        /whole-effect R32_SINT companion proof/u
    );
    const clonedProgram = particleIr(0, "reset");
    assert.throws(
        () => CjsFormatWebgpu.buildWgsl(
            clonedProgram,
